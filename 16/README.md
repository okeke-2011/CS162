...
```SQLite
BEGIN TRANSACTION;
INSERT INTO Orders VALUES (1002, 2000, "2025-01-21 10:00:00", 202501);
INSERT INTO OrderItems VALUES (1002, 3002, 500);
INSERT INTO SupplierOrders VALUES (6004,
     (SELECT SupplierID FROM (
        SELECT SupplierID, MIN(Cost) FROM SupplierProduct WHERE ProductID == 3002)
        ),
     3002, 4001, 500, "ORDERED", "2025-01-21", "2025-01-30");
...
```SQLite
BEGIN TRANSACTION;
UPDATE Customer SET AddressLine1 = ... WHERE CustomerID = 123;
UPDATE Customer SET AddressLine2 = ... WHERE CustomerID = 123;
UPDATE Customer SET AddressLine2 = ... WHERE CustomerID = 123;
END TRANSACTION;
```
...
```SQLite
BEGIN TRANSACTION;
DELETE FROM Product WHERE ProductID == 123;
DELETE FROM OrderItems WHERE ProductID == 123;
DELETE FROM Inventory WHERE ProductID == 123;
DELETE FROM SupplierProduct WHERE ProductID == 123;
DELETE FROM SupplierOrders WHERE ProductID == 123;
END TRANSACTION;
```
5. Critique the database design.  What data should be in the database schema,
but isn't?
I can see anything wrong with the current design
