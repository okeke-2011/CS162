...
- Queue elements in the queue are accessed in the order that they were put in. Ex: A line of people trying enter a building 
- Priority queue elements are accessed in order of some metric that serves as an indication of priority. Ex: When boarding, people with a higher ranking group board first
- In a LIFO queue (stack), the most recently entered elemnts are accessed first. Ex: In a stack of dishes, you take the plate on the top (the last plate you put on the stack). 
...
- We might need tests to make sure that requests from the users are delivered to the correct private chefs
- Test that the information entered by chefs and users is correctly shown on all other pages involving that information
