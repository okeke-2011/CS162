...
SELECT FIRSTNAME, SURNAME, COUNT(*) AS num_loans FROM Loans
    JOIN Clients ON Loans.CLIENTNUMBER = Clients.CLIENTNUMBER
...
SELECT '----------------------------------------------------';
SELECT SUM(BALANCE) FROM Loans WHERE STATUS NOT IN ("ARREARS", "Arrears");

SELECT '';
SELECT '5. Are all account numbers unique? (How should we fix this in general)';
SELECT 'Make the sql generate the account numbers itself (by incrementing)';

SELECT '';
SELECT '6. Martina has undergone gender reassignment and is now Martin';
SELECT '----------------------------------------------------';
UPDATE Clients SET FIRSTNAME = "Martin" WHERE CLIENTNUMBER = 4;
SELECT * FROM Clients;
...
SELECT Clients.EMAIL FROM Clients
    JOIN Loans ON Clients.CLIENTNUMBER == Loans.CLIENTNUMBER
    WHERE STATUS == "PAID OFF";

SELECT '';
SELECT '8. Print out the largest loan for each client';
SELECT '----------------------------------------------------';
SELECT MAX(PRINCIPALDEBT) FROM Clients
    JOIN Loans ON Clients.CLIENTNUMBER == Loans.CLIENTNUMBER
    GROUP BY(Clients.CLIENTNUMBER);
