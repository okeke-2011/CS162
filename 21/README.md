...
772ca35cb653   simple-cs162-flask:latest   "python app.py"   20 seconds ago   Up 20 seconds   0.0.0.0:5000->5000/tcp   simple-cs162-instance
```
...
- Not sure what this question is asking. All I know is that if the data is stored on a database and we rerun the application uisng a new container, all the data will still be there. However, if the data is stored on the server it's gone after the session is closed. 
...
- If we know specific information (like API keys) will be static throughout our app, it is better to store it in an environment variables instead of directly in the code base. It is also helpful if a developer wants to make use of a different API key for example. They don't need to change the API key value everywhere in the code base to get their code running. They just change one environment variable. 
